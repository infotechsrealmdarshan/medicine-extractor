...
export interface SearchResponse {
  ...
  failures: Partial<Record<'myp-i-n' | 'trident' | 'aah' | 'alliance', string>>;
}
...
export class SearchService {
  async search(query: string): Promise<SearchResponse> {
    ...
    try {
      ...
      const sources: Array<'myp-i-n' | 'trident' | 'aah' | 'alliance'> = ['myp-i-n', 'trident', 'aah', 'alliance'];
      ...
      return { products: [], failures: { 'myp-i-n': 'Unexpected search service failure', trident: 'Unexpected search service failure', aah: 'Unexpected search service failure', alliance: 'Unexpected search service failure' } };
    }
  }
}
